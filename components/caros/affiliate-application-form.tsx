...
export function AffiliateApplicationForm() {
  ...
  return <form onSubmit={submit} className="flex flex-col gap-8 rounded-2xl border border-border bg-card p-6 shadow-sm lg:p-8">
    ...
    <fieldset><legend className="mb-5 text-xl font-extrabold">7. {t("sections.found")}</legend><div className="grid gap-5"><label className="flex flex-col gap-2 text-sm font-semibold"><span>{t("form.source")}<span className="text-gold"> *</span></span><select required value={form.affiliate_program_source} onChange={(e) => { const value = e.target.value; setForm((current) => ({ ...current, affiliate_program_source: value, affiliate_referred_by: referralSources.includes(value) ? current.affiliate_referred_by : "" })) }} className="min-h-12 rounded-xl border border-border bg-background px-4 font-normal"><option value="">{t("form.choose")}</option>{[...referralSources,"CAROS website","Social media (Instagram, LinkedIn, etc.)","Email or newsletter","Webinar or online event","In-person event or conference","Other"].map((option) => <option key={option}>{option}</option>)}</select></label>{referralRequired && input("affiliate_referred_by", t("form.referredBy"), true)}</div></fieldset>
